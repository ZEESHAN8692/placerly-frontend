...
import { contact_us_end } from '../api/urls';
...
const ContactUsComp = () => {
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    setIsLoading(true)
    const response = await axiosInstance.post(contact_us_end,formData)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
          ...
          <div className="bg-white/5 border border-[#F8FAFC]/10 rounded-2xl p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                className="cursor-pointer w-full py-4 bg-gradient-to-r from-[#F9C74F] to-[#F9844A] text-[#0B1F3A] font-bold text-lg rounded-xl hover:shadow-2xl hover:shadow-[#F9C74F]/40 hover:scale-105 transition-all duration-300"
                disabled={isLoading}
              >
