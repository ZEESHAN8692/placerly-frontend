...
const InvestmentPage = () => {
  ...
  const { mutate: createMutate, isLoading: isCreating } = useMutation({
    ...
    onError: () => toast.error('Investment Creation Failed'),
  });
  ...
  const { mutate: updateMutate, isLoading: isUpdating } = useMutation({
    ...
    onError: () => toast.error('Investment Update Failed'),
  });
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.type || !formData.name || formData.amount === '' || !formData.date) {
    ...
    return (
      <DashboardLayout>
        <div className="min-h-screen flex items-center justify-center text-red-400">
          Error loading investments: {error?.message ?? 'Something went wrong'}
        </div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout>
      <div className="min-h-screen bg-gradient-to-br from-[#0B1F3A] via-[#0A1526] to-[#08101D] p-6 text-[#F8FAFC]">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold text-[#F9C74F]">Investments</h1>
            <p className="text-[#F8FAFC]/70">Manage all your investments easily</p>
          </div>
          <button
            onClick={() => openModal()}
        ...
        <div className="bg-[#0B192D] border border-[#F8FAFC]/10 rounded-xl overflow-hidden shadow-lg">
          <DataTable
            ...
            customStyles={{
              ...
              headCells: { style: { color: '#F9C74F', fontWeight: 'bold', fontSize: '15px' } },
              rows: {
                style: {
            ...
            <div className="bg-[#0B1F3A] border border-[#F8FAFC]/20 rounded-2xl p-6 w-full max-w-md relative">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <input
                  ...
                  placeholder="Enter Type (e.g., Bonds)"
