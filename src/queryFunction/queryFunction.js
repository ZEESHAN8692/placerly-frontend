...
export const getBlog = async () => {
    try {
        const res = await axiosInstance.get(get_blog_end);
        return res.data; 
    } catch (error) {
...
export const getBlogBySlug = async(slug)=>{
   try {
        const res =await axiosInstance.get(`${get_blog_by_slug}/${slug}`);
        return res.data;
...
export const addComment = async (id, payload) => {
  try {
    const res = await axiosInstance.post(`${add_comment_end}/${id}`, payload);
    return res.data;
  } catch (error) {
    console.log("BLOG API ERROR:", error);
    throw error;
